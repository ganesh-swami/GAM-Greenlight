...
import { Stack, Button, Col, Row, Dropdown } from "react-bootstrap";
...
export default function Room() {
  ...
  const { isLoading: isRoomLoading, data: room } = useRoom(friendlyId);
  const startMeeting = useStartMeeting(friendlyId);
  const currentUser = useAuth();
  const localizedTime = localizeDayDateTimeString(
    room?.last_session,
  ...
  function copyInvite(role) {
    ...
    } else if (role === "moderator-link") {
      navigator.clipboard.writeText(`${window.location}/join?ismoderator=1`);
              ...
              <Stack className="room-header-wrapper">
                <Stack direction="horizontal" gap={2}>
                  ...
                  <Stack direction="horizontal" className="mb-1">
                    ...
                    {room?.shared && (
                      <SharedBadge ownerName={room?.owner_name} />
                    )}
                  </Stack>
                </Stack>
                {room?.last_session ? (
                  <span className="text-muted">
                    {" "}
                    {t("room.last_session", { localizedTime })}{" "}
                  </span>
                ) : (
                  <span className="text-muted">
                    {" "}
                    {t("room.no_last_session")}{" "}
                  </span>
                )}
              </Stack>
            )}
              ...
              <Col className="col-12">
                ...
                <Dropdown className="btn-group mt-1 mx-2 float-end pb-5">
                  ...
                  <Dropdown.Menu className="dropdown-menu">
                    {roomSettings?.data?.glModeratorAccessCode && (
                      <Dropdown.Item
                        onClick={() => copyInvite("moderator-link")}
                      >
                        Copy moderator Link
                      </Dropdown.Item>
                    )}
