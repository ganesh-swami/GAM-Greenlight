...
import { Button, Col, Row, Stack, Form as RegularForm } from "react-bootstrap";
...
export default function JoinCard() {
  ...
  let isLinkForModerator = queryParams.has("ismoderator");

  useEffect(() => {
    // set cookie to return to if needed
    const date = new Date();
    date.setTime(date.getTime() + 60 * 10000); // expire the cookie in 10min
    document.cookie = `location=${path};path=/;expires=${date.toGMTString()}`;

    return () => {
      // delete redirect location when unmounting
  ...
  const handleJoin = (data) => {
    ...
    if (
      ...
      !methods.getValues("access_code")
    ) {
      return methods.setError(
        "access_code",
        { type: "required", message: t("room.settings.access_code_required") },
        { shouldFocus: true }
      );
    }

    if (
      publicRoom?.data.moderator_access_code &&
      !methods.getValues("access_code") &&
      isLinkForModerator
    ) {
      return methods.setError(
  ...
  useEffect(() => {
    ...
    if (roomStatusAPI.isSuccess) {
      ...
      const channel = subscribeToRoom(friendlyId, {
        onReceived: () => {
          setHasStarted(true);
        },
      });
      ...
      return () => {
        channel.unsubscribe();
        console.info(
          `WS: unsubscribed from room(friendly_id): ${friendlyId} channel.`
        );
      };
    }
  ...
  const notifyMeetingStarted = () => {
    ...
    const audio = new Audio(
      `${process.env.RELATIVE_URL_ROOT}/audios/notify.mp3`
    );
  ...
  useEffect(() => {
    ...
    if (hasStarted) {
      console.info(
        `Attempting to join the room(friendly_id): ${friendlyId} meeting.`
      );
  ...
  useEffect(() => {
    ...
    if (roomStatusAPI.isError) {
      ...
      if (roomStatusAPI.error.response.status === 403) {
        methods.setError(
          "access_code",
          { type: "SSE", message: t("room.settings.wrong_access_code") },
          { shouldFocus: true }
        );
      }
  ...
  if (
    ...
    publicRoom.data.require_authentication === "true"
  ) {
    return <RequireAuthentication path={path} />;
  }

  if (
    publicRoom.data.owner_id === currentUser?.id ||
    publicRoom.data.shared_user_ids.includes(currentUser?.id)
  ) {
    return <Navigate to={`/rooms/${publicRoom.data.friendly_id}`} />;
  }

  let hasAccessCode = false; //publicRoom.data?.viewer_access_code || publicRoom.data?.moderator_access_code;

  if (publicRoom.data?.moderator_access_code && isLinkForModerator) {
    hasAccessCode = true;
  } else if (!isLinkForModerator && publicRoom.data?.viewer_access_code) {
    hasAccessCode = true;
  } else if (
    publicRoom.data?.viewer_access_code &&
    publicRoom.data?.moderator_access_code
      ...
      <Card.Body className="pt-4 px-5">
        <Row>
          <Col className="col-xxl-8">
            ...
            <h1 className="mt-2">{publicRoom?.data.name}</h1>
          ...
          <Col>
            <Stack direction="vertical" gap={3}>
              <Avatar
                ...
                avatar={publicRoom?.data.owner_avatar}
                size="medium"
              />
          ...
          {roomStatusAPI.isSuccess && !roomStatusAPI.data.status ? (
            ...
            <Form methods={methods} onSubmit={handleJoin}>
              <FormControl
                field={fields.name}
                type="text"
                disabled={currentUser?.signed_in}
          ...
          {signInOnRoomJoin &&
            ...
            (env?.EXTERNAL_AUTH ? (
              <Stack
                ...
                className="d-flex justify-content-center text-muted mt-3"
              >
                {" "}
                ...
                >
                  ...
                  <Button
                    variant="link"
                    className="btn-sm fs-6 cursor-pointer ms-2 ps-0"
                    type="submit"
                  >
              ...
              <div className="text-center text-muted mt-3">
                ...
                {t("authentication.already_have_account")}
                <Link
                  to={`/signin?location=${path}`}
                  className="text-link ms-1"
                >
                  {" "}
                  {t("authentication.sign_in")}{" "}
                </Link>
